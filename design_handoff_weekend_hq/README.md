...
3. **Hero action / conflict callout** (only renders if the day has one)
   - Soft orange tinted card with hairline orange border.
   - Collapsible. Header row: warning icon + bold orange title `Saturday morning handoff` + chevron.
   - When open: a paragraph of body, then a numbered step list (1, 2, 3, 4). Numbers are 18px solid orange circles with white digits.
...
4. **Dual-lane timeline** — the centerpiece
   ...
   - **Vertical scale:** 1.5 px per minute on mobile, 1.2 px per minute on desktop. So a 90-minute game card is ~135px tall on mobile.
   - **Game cards** sit inside their lane column. Width = half the lane area minus gutters. White surface, hairline border in the kid's bg color, 3px solid colored left edge, radius 10. Content: time + field code (top row), `Game 1` (small bold kid-color), `vs <short opponent>` (12px ink2, line-clamped to 2 lines).
   - When `attending: false` (i.e. Max's Game 1 — the conflict): card opacity drops to 0.62 and a small `· NOT ATTENDING ·` mono caps tag appears.
   ...
   - **Conflict overlay**: where two games overlap (Sat 8:00–9:15), draw a translucent diagonal-stripe rectangle behind both cards, dashed warm-orange border, with a small caps tag in the top-right: `YOU CAN'T BE AT BOTH`.
   ...
   - **Tap behavior**: tapping a game card expands it in place, pushing the rest of the timeline down. Expanded view shows full opponent name, age group, full venue name + field + full address, optional italic note in a tinted box, and a solid-color `Open in Maps →` button. Tap again to collapse.
...
A different top bar (`Venues` as the big title, same eyebrow row), then a vertical list of two venue cards:

- Each card has a tinted header strip in the kid's color (dot + kid name in caps), then white body with venue name (16px 600), address (12px muted, linked).
- Below: a vertical list of time slots. Each slot is a tinted row: day badge (mono, 2-char), time (mono), field name. E.g. `SAT  9:15 AM  Field 01B`.
- A solid dark `Directions →` button.

Footer card: muted "Drive between venues" block with the 20-min drive note.
...
- **OPEN ITEMS** — three rows: `Ben (Sofia's coach)`, `Simon (Max's coach)`, `You`. Each row has the who in caps mono, then the what.
...
Color usage rules:
- **Sofia gets rose, Max gets teal.** This is consistent throughout. Avoid pink/blue cliché.
- Orange is reserved for **alerts and now** — never decorative.
...
TOURNAMENT = {
  ...
  days: [
    {
      id, label, dow, date,
      startHour, endHour,           // timeline bounds
      events: [
        { id, kind: 'game' | 'travel', lane: 'sofia' | 'max' | 'both',
      ...
      conflicts: [ { start, end, label } ],
      heroAction: { title, body, steps: [...] } | null
    }
  ],
  venues: [...],
  driveNote,
...
- Triangle warning (conflict callout)
- Car (travel chips)
